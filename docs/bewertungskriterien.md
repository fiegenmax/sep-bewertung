...
**LLM ergänzt (Sonnet):** Die Heuristik prüft nur Repo-*Hygiene* auf Datei-Ebene und liest den Code nie. Der LLM-Call (`analyze_code_quality_llm`) schließt genau diese Lücke: Er sampelt die **größten** Produktions-Source-Files (Test- und Vendor-Dateien ausgeschlossen, Default 6 Dateien / je 2500 Zeichen / 12k gesamt — alles unter `llm_sampling.code_quality_*`) und beurteilt die tatsächliche Code-Qualität auf einer 0–3-Skala: größere Mängel wie God-Classes, Copy-Paste-Duplikation, toter/auskommentierter Code, verschluckte Exceptions, irreführende Namen, fehlende Trennung von Verantwortlichkeiten. Stil-Nitpicks bleiben bewusst außen vor. Modell: `llm.code_quality_model` (Default Sonnet, weil Code-Verständnis zählt; auf Haiku setzbar). Der 0–3-Score wird beim Excel-Schreiben auf die 0–1-Kriteriumsskala normalisiert (Spalte D). Heuristik (Hygiene) und LLM (Qualität) sind komplementär und beeinflussen sich auf der Datenebene nicht.

**Blinde Flecken:** Die Heuristik gibt 0/1 — kein Halbwert. Ein Repo mit nur einer .DS_Store-Datei bekommt die gleiche Bewertung wie eins mit 100 MB node_modules + 200 TODOs. Und: Der TODO-Zähler ist Java/TS-spezifisch — ein reines Python-/Go-Team wird bei TODOs nicht erfasst (die übrigen Checks sind dagegen `git ls-files`-basiert und damit sprachunabhängig). Der LLM-Call mildert das, da er sprachübergreifend echten Code liest — sieht aber nur eine größenbasierte Stichprobe, nicht das ganze Repo.
